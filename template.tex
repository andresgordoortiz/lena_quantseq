...
The raw output files for the pipeline can be found in \verb|/bioinfo/andres.gordo/rnaseq_nfcore_out_umi.tar.gz|. The main output file that was used for downstream analysis is the gene counts matrix \verb|salmon.merged.gene_counts.tsv|. Normalisation and differential expression analysis was performed using DESeq2 in R (see code for details). From now onwards, a differentially expressed gene (DEG) is defined as one with an adjusted p-value $< 0.05$ (BH) and an absolute log2 fold change $\geq 1.0$. Genes with less than 10 counts across at least 3 samples were filtered out prior to DE analysis. This, as will be observed later, resulted in wnt11---member of the Nodal score---being lost. The variance was not stabilised, as I wanted to keep the log2 fold changes as close to the raw counts as possible.

\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{compare_annotations/annotation_comparison_figure.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{q1_combined_volcano.pdf}
\caption{Volcano plots of DEGs in each experiment. In purple the shared genes. DEGs defined as adjusted p-value $<= 0.05$ and absolute log2 fold change $>= 1.5$.}
...
Finally, I performed a quick Gene Ontology enrichment analysis using the clusterProfiler package in R. I focused on the Molecular Function ontology and explored the top enriched terms for the different comparisons performed so far. Figure~\ref{fig:go_mf} shows a selection of interesting terms that were enriched in different comparisons. The Exp~1 figure shows the Gene Ontology terms of the DEGs of each Activin concentration at each timepoint compared to the non-induced control samples. It shows predominancy of terms related to growth, signalling pathways, and transcription. Interestingly, the terms related to signalling receptors appear in every condition except for the latest timepoint, regardless of concentration, suggesting a negative feedback loop whereby Activin at first prompts the transcription of signalling receptors, but later on these get downregulated again.

The Exp~2 figure shows the GO terms enriched in the DEGs obtained when comparing Activin vs.\ control at endpoint (left), SB50 treatment vs.\ the control (centre), and SB50 treatment vs.\ Activin (right). Signalling receptor terms do not appear in the Activin effect, consistent with the idea that at the latest timepoint (240~min) Activin has already downregulated those receptors. Following this hypothesis, SB50 compared to the baseline control only shows enrichment of signalling receptors when adding it at 60 minutes, since that is when we have both the maximum Activin-induced phenotype inhibition and the (according to Exp~1 GO) the receptors might be more sensitive to Activin. Adding SB50 later does not inhibit Activin that much, thereby not altering the downregulation of receptors at endpoint (240~min).

Lastly, when we uncouple the SB50 inhibition from the Activin induction (rightmost), things get a bit more tricky from my perspective. Here is my take on this: at 60 minutes SB50 is effectively inhibiting Activin, which would lead to an increase in signalling receptor genes as seen in the vs.\ baseline comparison. However, since we are comparing to the Activin control, which might have some of these receptors being expressed, albeit less than in previous timepoints, the net effect is no enrichment. Comparing the SB50 to the Activin control, we can uncouple the natural regulatory network of Activin on these receptors from its inhibition. At 180 minutes, the inhibiting effect of SB50 is minimal, therefore we are effectively comparing Activin at endpoint with Activin at endpoint, leading to no enrichment---not only in receptor signalling, but in any term. However, because the later we add SB50, the lesser its effect, one would expect that adding SB50 at 120 minutes would lead to an even less intense Activin inhibition, and therefore no enrichment of receptors. If at 60 minutes SB50 cannot rescue the receptors at endpoint, adding it at later stages should be even less effective.

However, we do see an enrichment of receptor signalling terms at 120 minutes. That led me to explore the direction of enrichment, meaning: Are the genes driving GO enrichment upregulated or downregulated? Figure~\ref{fig:receptors_go} shows that information.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{go_mf_combined_publication.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{exp2_go_genes_direction.pdf}
\caption{Direction of gene expression changes for genes driving the `signalling receptor activity' GO term enrichment in Experiment 2 comparisons.}
\label{fig:receptors_go}
\end{figure}
